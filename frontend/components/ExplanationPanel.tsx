...
const ConfidenceGauge: React.FC<{ score: number; label: string }> = ({
  score,
  label,
}) => {
...
// Custom Tooltip to avoid Recharts formatter type issues entirely
const CustomTooltip = ({
  active,
  payload,
  label,
  ...
  active?: boolean;
  payload?: Array<{ value: number; payload: { description: string } }>;
  label?: string;
}) => {
  ...
  return (
    ...
    >
      <p style={{ color: "#f1f5f9", margin: 0, fontWeight: 600 }}>{label}</p>
      <p style={{ color: "#60a5fa", margin: "4px 0 0" }}>{value}%</p>
      {description && (
...
export default function ExplanationPanel({ explanation, isLoading }: Props) {
  const [activeTab, setActiveTab] = useState<
    "reasoning" | "features" | "recommendations"
  >("reasoning");
  ...
  const risk =
    explanation.risk_assessment?.overall_risk ||
    explanation.risk_level ||
    "low";
  const riskStyle = RISK_COLORS[risk] || RISK_COLORS.low;

  const chartData = (explanation.feature_contributions || [])
    .slice(0, 6)
    .map((f) => ({
      ...
      <div style={{ padding: "16px 20px", minHeight: "160px" }}>
        {activeTab === "reasoning" && (
          <div>
            ...
            {/* Risk items */}
            {explanation.risk_assessment?.risks &&
              explanation.risk_assessment.risks.length > 0 && (
            ...
            <ResponsiveContainer width="100%" height={160}>
              ...
              >
                ...
                <YAxis
                  ...
                  width={100}
                />
                {/* Use custom tooltip to avoid Recharts formatter TypeScript issues */}
