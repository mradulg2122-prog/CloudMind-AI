...
#   4. Apply the decision engine to recommend server scaling
# =============================================================================
...
try:
    ...
    # If model file is missing, we raise a clear error so the developer knows
...
def decision_engine(predicted_requests: float, current_servers: int) -> dict:
    """
    Decide how many servers are needed and what action to take.

    Logic:
    ──────
    • Load per server = predicted_requests / current_servers
    • If load > 300 req/server  → SCALE UP   (add servers)
    • If load < 120 req/server  → SCALE DOWN (remove servers)
    • Otherwise                 → KEEP SAME

    Thresholds are based on the assumption that each server handles
    ~200 req/min comfortably. 300 = overloaded, 120 = underutilised.
    ...
    Returns
    -------
    dict with keys: recommended_servers, action, load_per_server
    """
    ...
    if load_per_server > 300:
        # Overloaded — calculate how many servers needed at target load of 200
        recommended_servers = max(current_servers + 1,
                                  int(np.ceil(predicted_requests / 200)))
        action = "SCALE UP"

    elif load_per_server < 120:
        # Underutilised — scale down but keep at least 1 server running
        recommended_servers = max(1,
                                  int(np.ceil(predicted_requests / 200)))
        action = "SCALE DOWN"

    else:
        # Load is healthy — no change needed
...
def run_prediction(data: dict) -> dict:
    ...
    Full prediction pipeline:
      ...
      3. Run decision engine to get scaling recommendation
